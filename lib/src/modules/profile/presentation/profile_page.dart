...
class _ProfilePageState extends State<ProfilePage> {
  ...
  Future<void> _openEditProfile(BuildContext context) async {
    ...
    // ⭐ LUÔN TRẢ VỀ String (không nullable) VÀ ƯU TIÊN STATE LOCAL
    final String currentUsername = _displayUsername ??
        profileState.maybeWhen(
          loaded: (profile) => profile.username,
          orElse: () => user.username ?? user.email ?? '',
        );

    final String? currentBio = _profileBio ??
        profileState.maybeWhen(
          loaded: (profile) => profile.bio,
          orElse: () => null,
        );

    final String? currentAvatar = _avatarUrl ??
        profileState.maybeWhen(
          loaded: (profile) => profile.avatarUrl,
          orElse: () => null,
        );

    // 👇 Mở màn Edit với dữ liệu MỚI NHẤT
    await context.router.push(
      EditProfileRoute(
        initialUsername: currentUsername, // ✅ giờ là String
        initialBio: currentBio, // String?
        initialAvatarUrl: currentAvatar, // String?
      ),
    );

    // Khi EditProfile pop về → reload lại từ Supabase + cubit
    if (!mounted) return;
